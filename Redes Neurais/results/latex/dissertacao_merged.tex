...
O limiar usado como referência analítica neste trabalho é obtido a partir da mesma derivação de Xie et al.~\cite{ref_xie}, mas com a energia da TAG efetivamente gerada pelo mapa da tenda. Essa adaptação é necessária porque Xie normaliza a energia da sequência para $1$, enquanto a TAG deste trabalho satisfaz, após a normalização usada no \textit{nn01},
\begin{equation}
...
\end{equation}
Impondo $P_{FA}(u_0)=\alpha$ e invertendo essa expressão, resulta o limiar teórico Rayleigh--médio usado no \textit{nn02c}:
\begin{equation}
...
O mesmo procedimento fornece o $P_D$ teórico. Definindo $d=u_0-L_{\mathrm{eff}}$ e usando $P_D=1-P(\tau<u_0\mid H_1)$, obtém-se
\begin{equation}
...
\end{equation}
Quando $u_0<L_{\mathrm{eff}}$, o termo $\operatorname{sgn}(d)$ torna a probabilidade de detecção maior que $1/2$, como esperado; quando o limiar fica acima da média sob $H_1$, a detecção diminui. No código do \textit{nn02c}, as Equações~\eqref{eq:limiar_tag} e~\eqref{eq:pd_teorico_tag} são implementadas pela função \texttt{xie\_closed\_form\_rayleigh}, chamada com $\sigma_t^2=1/3$, $\alpha=0{,}01$, o $L_{\mathrm{FIXED}}$ armazenado no dataset e o $\rho_t$ armazenado no mesmo arquivo.
...
Na validação Monte Carlo do \textit{nn01}, são geradas TAGs e ganhos Rayleigh independentes. Para cada realização, amostra-se uma estatística gaussiana com média $0$ sob $H_0$ ou média $t_2=\sum_k t_k^2$ sob $H_1$, e variância condicionada a $h^2$. Com o mesmo limiar $u_0$, calculam-se
\begin{align}
    ...
    \label{eq:pd_mc}
\end{align}
O Monte Carlo testa a implementação e as hipóteses estatísticas da fórmula, mas possui erro amostral e, na validação do \textit{nn01}, usa $t_2$ médio para determinar o limiar. Portanto, uma pequena diferença em relação a $P_D^{\mathrm{teo}}$ é esperada; uma diferença sistemática aponta para comprimento variável, energia finita da TAG, normalização, convenção de SNR ou outro desvio do modelo.

\paragraph{(c) $P_D$ empírico do correlator no \textit{nn02c}.}
No gráfico principal, a série ``correlador clássico (empírico, D3F threshold)'' não é a curva fechada de Xie. Para cada bin de SNR, o notebook estima no conjunto de validação sob $H_0$ a média e o desvio-padrão de $|\tau|$ e define
\begin{equation}
...
O NN\_02c avaliou diretamente a suficiência do sinal residual de dimensão 1024. A Tabela~\ref{tab:zeq_ablation} sintetiza a comparação final entre o correlator clássico e as Variantes D e G, usando os resultados exportados em \texttt{nn02c\_ablation\_dnn\_results.json}.

\begin{table}[htb]
\caption{\label{tab:zeq_ablation}NN\_02c --- comparação final das Variantes D e G. $P_D$ (\%) sob $\alpha=0{,}01$, $L_{\mathrm{FIXED}}=64$, com $L\in[32,64]$ e preenchimento por zeros, em canal Rayleigh.}
\begin{center}
{\footnotesize
...
\begin{figure}[htb]
    ...
    \caption{\label{fig:nn02c_pd_snr}NN\_02c: comparação de $P_D$ versus SNR entre o correlador clássico e as Variantes D e G. A Variante D recebe $z_{eq}$ e $|\tau|$; a Variante G recebe o produto elemento a elemento $z_{eq}\odot tag_{ref}$.}
\end{figure}
