...
\subsection{Limite de Otimalidade de Neyman-Pearson e o Papel do CFAR Adaptativo}\label{sec:np_limit}

Pelo lema de Neyman-Pearson~\cite{ref_poor}, o teste de razão de verossimilhança (LRT) construído a partir de $\tau^i$ na Equação~\eqref{eq:tau} é o detector ótimo --- no sentido de maximizar $P_D$ para qualquer $P_{FA}$ fixa --- \emph{desde que o ganho de canal instantâneo $h^i$ seja conhecido ou perfeitamente estimado e utilizado na normalização do próprio $\tau^i$}. Sob essa condição, nenhum classificador, incluindo redes neurais de qualquer arquitetura, pode produzir $P_D$ estritamente maior que o correlador na mesma $P_{FA}$: trata-se de um limite teórico, não de uma limitação de implementação.

Esse resultado exige uma leitura cuidadosa dos ganhos observados neste trabalho, particularmente do núcleo mínimo $[|\tau|,\hat{h}]$, que atinge $84{,}7\%$ de detecção a $0$~dB frente a apenas $8{,}1\%$ do correlador clássico de referência (Tabela~\ref{tab:zeq_ablation}). A superioridade aparente não viola o lema de Neyman-Pearson porque as duas estatísticas comparadas não resolvem exatamente o mesmo teste de hipótese. O correlador clássico de referência, conforme tipicamente implementado e comparado na literatura~\cite{ref_xie}, calibra um limiar único e fixo por faixa de SNR nominal, obtido a partir da média/agregado das amostras daquela faixa --- e não da realização instantânea $h^i$ de cada pacote. Como $h^i$ segue distribuição de Rayleigh e varia livremente dentro de uma mesma faixa nominal de SNR, esse procedimento resolve, na prática, um teste de hipótese simples sob um canal médio ou nominal, e não o teste de hipótese composto real, em que $h^i$ é um parâmetro de perturbação (\textit{nuisance parameter}) aleatório e desconhecido a cada pacote.

A rede neural que recebe $\hat{h}^i$ como entrada auxiliar, por sua vez, pode aprender uma fronteira de decisão condicionada ao valor instantâneo do canal --- aproximando-se, na prática, de um teste do tipo \textit{Generalized Likelihood Ratio Test} (GLRT) com adaptação CFAR por pacote. Esse teste é uma solução mais informada e melhor especificada para o problema composto e, por isso, pode superar a implementação de referência do correlador clássico sem violar a otimalidade de Neyman-Pearson: a rede não supera o detector ótimo do problema completo, ela supera uma implementação subótima que ignora informação instantânea de canal disponível. Um correlador clássico ``justo'', com limiar adaptado a $\hat{h}^i$ por amostra (um correlador CFAR análogo ao proposto por~\cite{ref_xie}), deveria recuperar paridade com a rede neural, e não inferioridade frente a ela --- consistente com o limite teórico estabelecido no parágrafo anterior.

Essa distinção também explica os resultados da ablação $z_{eq}$ (Seção~\ref{sec:ablation_zeq}). Como o vetor $z_{eq}$ e a tag de referência $\text{tag}_{ref}$ contêm, em conjunto, toda a informação necessária para computar $\tau^i$ (um produto interno linear, computável por um único neurônio denso), o teto teórico de desempenho das Variantes A, B, C e E é exatamente a paridade com o correlador quando o canal é conhecido, e nunca sua superação. O desempenho substancialmente inferior observado nessas variantes em baixo SNR reflete, portanto, uma dificuldade prática de estimação estatística --- aprender a direção de correlação correta em um espaço de 1024 dimensões dominado por ruído a partir de dados finitos --- e não uma limitação fundamental de informação. A Variante D, ao fornecer $|\tau|$ já computado, elimina essa dificuldade de otimização e recupera o teto teórico esperado.

Por fim, esse enquadramento delimita precisamente a contribuição pretendida deste trabalho. Em canais com estatística de ruído conhecida e gaussiana (o cenário Rayleigh avaliado até aqui), a rede neural pode, no máximo, igualar o correlador ótimo quando devidamente informada sobre o canal instantâneo --- e o ganho reportado decorre da comparação com uma referência clássica que não adapta seu limiar por pacote, e não de uma superação do limite de Neyman-Pearson. A vantagem genuína e duradoura de uma abordagem orientada a dados surge apenas em cenários nos quais a distribuição do ruído é desconhecida ou não-gaussiana --- por exemplo, o Cenário 3 do \textit{D3F}~\cite{ref_braca} --- onde não existe forma fechada para o LRT e o correlador clássico deixa de ser ótimo por depender de premissas paramétricas que não se sustentam. Nesse regime, o detector aprendido diretamente dos dados pode superar genuinamente qualquer heurística clássica, e não apenas equalizar uma implementação de referência subótima.
...
\begin{table}[htb]
\caption{\label{tab:tau_ablation}NN\_02b --- variantes de entrada baseadas em $\tau$, com classificação de paridade informacional frente ao clássico. $P_D$ (\%) sob $\alpha=10^{-7}$.}
\begin{center}
{\footnotesize
...
\subsection{Ablação z\_eq (NN\_02c)}\label{sec:ablation_zeq}
...
Clássico ($|\tau|$ analítico) & Ref. & 8{,}1 & 58{,}7 & 99{,}9 & 100{,}0 & 100{,}0 & 100{,}0 & 100{,}0 \\\midrule
A: z\_eq somente             & P+CI & 0{,}4 & 0{,}1 & 0{,}0 & 86{,}2 & 100{,}0 & 100{,}0 & 100{,}0 \\
B: z\_eq + h                 & S    & 0{,}2 & 0{,}1 & 0{,}0 & 88{,}0 & 100{,}0 & 100{,}0 & 100{,}0 \\
C: z\_eq + h + SNR           & S    & 0{,}0 & 0{,}0 & 0{,}0 & 99{,}3 & 100{,}0 & 0{,}0 & 0{,}0 \\
D: z\_eq + $|\tau|$          & P+CI & \textbf{88{,}3} & \textbf{99{,}7} & 100{,}0 & 100{,}0 & 100{,}0 & 100{,}0 & 100{,}0 \\
E: z\_eq - tag\_ref          & P+CI & 0{,}2 & 0{,}2 & 0{,}0 & 0{,}0 & 100{,}0 & 100{,}0 & 100{,}0 \\
\bottomrule
\end{tabular}
}
\end{center}
